...
#model_list_filenames = glob.glob('.\{}_experiments_{}_10_models\models_list_*0.001.pkl'.format(problem, model_name))
metrics_filenames = glob.glob('.\{}_experiments_{}_10_models\metrics_*0.001.pkl'.format(problem, model_name))
loss_history_filenames = glob.glob('.\{}_experiments_{}_10_models\loss_history_*0.001.pkl'.format(problem, model_name))
...
for lh_filename, metrics_filename in zip(loss_history_filenames, metrics_filenames):
    ...
    fig.suptitle(model_name + " " + problem + " " + params[0] + "," + params[1] + "," + params[2])
        ...
        with open(metrics_filename, 'rb') as metrics_file:
            ...
            if model_name=="DON":
                ...
                for i, (lh, lh_v) in enumerate(zip(lhs['train'], lhs['validation'])):
                    ...
                    r2 = metrics["R2"][i].item()
                    print(i, r2)
                    axs[0].plot(np.arange(len(lh)), lh, linewidth=0.5, label="{:.2f}".format(r2)); axs[0].set_yscale("log"); axs[0].set_ylim([1e-4, 1e1]); axs[0].set_title("Train loss")
                    axs[1].plot(np.arange(len(lh_v)), lh_v, linewidth=0.5); axs[1].set_yscale("log"); axs[1].set_ylim([1e-4, 1e0]); axs[1].set_title("Validation loss")
            print(metrics)
