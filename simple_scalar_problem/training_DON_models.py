...
n_train = 300 # no. of training samples
...
branch_hidden_sizes = [100, 200]#, 400]
...
weight_penalties = [0.]#, 1e-3, 1e-2] # L2 penalty for NN weights
        ...
        for m in range(n_models):
            ...
            for epoch in range(iterations):
                ...
                for i in range(num_batches):
                    optimizer.zero_grad()
                    j = i*batch_size; k = (i+1)*batch_size
                    u_b = u_train[j:k]; x_b = x_train[j:k]; y_b = y_train[j:k]
                    output = model(u_b, x_b)
                    loss = loss_fn(output,y_b ,u_b,x_b)
            ...
            loss_test.append(loss_fn(preds, y_test, u_test, x_test).item())
            print()
