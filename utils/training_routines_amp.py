...
def validation_step(model, loss_fns, val_dataloader, device):
    # val_data = (u,x,y) for DON or (u,y) for FNO
    val_loss = 0.
    for val_data in val_dataloader:
        val_data = [tens.to(device) for tens in val_data]
        targets = val_data[-1]
        #print("val targets device", targets.device, targets.dtype)
        inputs = val_data[:-1]
        #u, r = inputs
        with autocast():
            #print("val inputs", inputs[0].shape, inputs[0].dtype)
            preds = model(*inputs)
            for loss_fn in loss_fns:
                val_loss += loss_fn(preds, targets, *inputs).item()
...
def train_FNO(model,
    ...
    def training_loop(model, optimizer, dataloader):
        ...
        for batch in dataloader:
            ...
            with autocast():
                # Compute forward pass and loss in mixed precision
                print("autocast u dtype", u.dtype)
                preds = model(u)
                loss = loss_fn(preds, y)
            print("lossdtype", loss.dtype)
    ...
    best_val_loss = validation_step(model, (lambda preds, targets, inputs: loss_fn(preds, targets),), dataset_val, device)
    best_epoch = 0
    ...
    for epoch in range(iterations):
        ...
        val_loss = validation_step(model, (lambda preds, targets, inputs: loss_fn(preds, targets),), dataset_val, device)
...
def train_DON(model,
    ...
    scaler = GradScaler()
    best_val_loss = validation_step(model, (loss_fn_data, loss_fn_physics), dataset_val, device)
    best_epoch = 0
    best_model_params = model.state_dict()
    ...
    def training_loop(model, optimizer):
        ...
        for i in range(n_fun_batches):
            ...
            for j in range(n_loc_batches):
                ...
                with autocast():
                    #print("trainin u dtype", u_batch.dtype)
                    preds = model(u_batch, x_batch)
                    #print("trainin preds", preds.dtype)
    ...
    for epoch in range(iterations):
        ...
        model.eval()
        # compute validation loss
        val_loss = validation_step(model, (loss_fn_data, loss_fn_physics), dataset_val, device)
        loss_history_val.append(val_loss)
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            best_epoch = epoch
